...
// The Stepper pins
#define STEPPER1_DIR_PIN 18  // Arduino A3
...
// PID control gains
static float vertical_kp = 200;   // proportional gain 450 //200//315//300
static float vertical_kd = 300;   // differential gain 450//290//375//350
static float velocity_kp = 0.04; // proportional gain 0.075//0.08//0.015//0.017
static float velocity_ki = velocity_kp / 200;
static float turn_kp = 0.0; // 0.5
static float turn_kd = 0.0;  // 1
static float turn_speed = 0.0;
static float turn_direction = 0.0; // either +-1 based on direction

// vertical loop
static float pitch;
static float bias = 0.08;
...
static float target_velocity = -1; // 1.3//1.15
...
void handleSetVariable(AsyncWebServerRequest *request) {
  if (request->hasArg("variable") && request->hasArg("value")) { 
    String variable = request->arg("variable");
    float value = request->arg("value").toFloat();

    if (variable == "vertical_kp") 
    {
      vertical_kp = value;
    } 
    else if (variable == "vertical_kd") 
    {
      vertical_kd = value;
    } 
    else if (variable == "velocity_kp") 
    {
      velocity_kp = value;
      velocity_ki = velocity_kp / 200;
    } 
    else if (variable == "velocity_ki") 
    {
      velocity_ki = value;
    } 
    else if (variable == "turn_kp") 
    {
      turn_kp = value;
    } 
    else if (variable == "turn_kd") 
    {
      turn_kd = value;
    } 
    else if (variable == "turn_speed") 
    {
      turn_speed = value;
    } 
    else if (variable == "turn_direction") 
    {
      turn_direction = value;
    } 
    else if (variable == "target_velocity") 
    {
      target_velocity = value;
    } 
    else if (variable == "target_angle") 
    {
      target_angle = value;
    } 
    else if (variable == "bias") 
    {
      bias = value;
    } 
    else if (variable == "yaw_bias") 
    {
      yaw_bias = value;
    } 
    else 
    {
      ...
      Serial.println("Invalid variable name received."); 
      return;
    }

    request->send(200, "text/plain", "Variable set: " + variable + " = " + String(value));
    Serial.printf("Variable set successfully: %s = %f\n", variable.c_str(), value);
  } 
  else 
  {
    request->send(400, "text/plain", "Invalid Request");
    Serial.println("Invalid request: missing variable or value");
  }
}
...
float veloctiy(float step1_velocity, float step2_velocity)
{
  ...
  float a = 0.7; // low pass filter coefficient
...
float turn(float gyro_x, float yaw)
{
  ...
  if (false)
  {
   output = (cam_rho+cam_theta)* turn_kp +gyro_x * turn_kd;
  }
...
//   if (tracking_mode){
//     output = (PI/2 - cam_theta) * turn_kp - gyro_x * turn_kd;
//   }
//   // {
//   //   read_sensor_values();
//   //   if (decide == 3)
...
//   //     // if(already_set == false)
//   //     // {
//   //     //   target_angle = yaw + 15/180*3.1415926;
//   //     //   already_set = true;
//   //     // }
//   //     // output = (target_angle - yaw) * turn_kp - gyro_x * turn_kd;
//   //   }
...
void loop()
{
  ...
  if (millis() > loopTimer_inner)
  {
    ...
    static unsigned long currents  = millis();
    previous = currents;
    static unsigned long LoopTime = previous - currents;
    ...
    {
      step1.setAccelerationRad(acc_input2);
      step2.setAccelerationRad(acc_input1);
    ...
    if (millis() > printTimer)
    {
      ...
      // Serial.print("cam theaa ");
      // Serial.println(90-(cam_theta ) / 3.1415926 * 180 / 10,6);
      // Serial.print("yaw: ");
